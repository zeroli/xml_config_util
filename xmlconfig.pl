...
sub Save {
	...
	my %defopts = (rootname=>"", suppressempty=>1, noattr=>1, keyattr=>[], contentkey=>"", nosort=>1);
	while (my ($key, $value) = each(%opts)) {
		$defopts{$key} = $value;
	}
...
sub Hash2Xml {
	my $hashref = shift;
	my $xml = XMLout($hashref, rootname=>"", suppressempty=>1, noattr=>1, keyattr=>[], contentkey=>[], nosort=>1);
	return $xml;
}

sub Xml2Hash {
	my $xml = shift;
	my $hashref = XMLin($xml, keeproot=>1, searchpath =>".", suppressempty=>1, noattr=>1, keyattr=>[], contentkey=>"");
	return $hashref;
}
