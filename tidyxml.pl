...
MyXML - A simple-to-use XML module, for parsing and creating XML documents
...
=head1 SYNOPSIS
    ...
    use MyXML qw(tidy_xml xml_to_object);
    use MyXML qw(:all);
...
sub tidy_xml {
	my $xml = shift;
	if ($xml eq 'MyXML') { confess "Error: 'tidy_xml' is a function, not a method"; }
	my $flags = shift || {};
...
Creates an 'MyXML::Object' object from the raw XML provided
...
sub xml_to_object {
	...
	my @stack;
	my $object = bless ({ content => [] }, 'MyXML::Object');
	my $pointer = $object;
	foreach my $el (@els) {
		if ($el =~ /^<\/?>$/) {
...
Creates an XML string from the 'MyXML::Object' object provided
...
sub _tidy_object {
	...
	$object->{'content'} = [];
	for my $i (0..$#children) {
		push @{$object->{'content'}}, bless ({ value => "\n".($flags->{'indentstring'}x($tabs+1)), parent => $object }, 'MyXML::Object');
		push @{$object->{'content'}}, $children[$i];
	}
	push @{$object->{'content'}}, bless ({ value => "\n".($flags->{'indentstring'}x($tabs)), parent => $object }, 'MyXML::Object');
...
sub _objectarray_to_simple_hashref {
	...
	foreach my $stuff (@$object) {
		if (defined $stuff->{'element'}) {
			...
			if ($flags->{'strip_ns'}) { $key = &MyXML::_strip_ns($key); }
			...
			if ($flags->{'strip'}) { $value = &MyXML::_strip($value); }
...
sub _objectarray_to_simple_arrayref {
	...
	if (ref $flags ne 'HASH') { confess "Error: This method of setting flags is deprecated in MyXML v0.083 - check module's documentation for the new way"; }
...
sub new {
	...
	my $obj = MyXML::xml_to_object($xml);
	bless $obj, $class;
	return $obj;
}
...
Returns the element specified by the path as an MyXML::Object object. When there are more than one tags with the specified name in the last step of the path, it will return all of them as an array. In scalar context will only return the first one.
...
Returns a very simple hashref, like the one returned with &MyXML::xml_to_simple. Same restrictions and warnings apply.
...
sub simplify {
	...
	my $simple = &MyXML::_objectarray_to_simple([$self], $flags);
	if (! $flags->{'internal'}) {
		return $simple;
	} else {
...
sub to_xml {
	...
	my $xml = &MyXML::_objectarray_to_xml([$self]);
	if ($flags->{'tidy'}) { $xml = &MyXML::tidy_xml($xml, { %$flags, complete => 0, save => undef }); }
	$xml = $decl . $xml;
	if ($flags->{'utf8'}) { Encode::_utf8_on($xml); }
